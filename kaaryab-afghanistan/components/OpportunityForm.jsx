...
export default function OpportunityForm({ initialData = {}, onSubmit }) {
  ...
  const inputCls = (err) =>
    `w-full rounded-xl border bg-white dark:bg-slate-800 px-4 py-3 text-slate-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${
      err ? "border-red-400 focus:ring-red-500/40 focus:border-red-500" : "border-slate-300 dark:border-slate-700"
    }`;

  const errorCls = "mt-1.5 text-xs font-medium text-red-600 dark:text-red-400";

  return (
    <form
      noValidate
      onSubmit={handleSubmit(submit)}
      className="space-y-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 shadow-lg"
    >
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <input
              ...
              type="date"
              min={todayStr()}
              className={`appearance-none pl-4 pr-12 ${inputCls(errors.deadline)}`}
            />
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3 text-slate-500 dark:text-slate-400"
            >
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          className={`min-h-[140px] rounded-2xl ${inputCls(errors.description)}`}
        />
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{t("form.commaSeparated")}</p>
        </div>
        <div className="space-y-2">
      ...
      <button
        ...
        className="w-full rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-700 px-6 py-4 text-white font-bold hover:from-blue-700 hover:to-indigo-800 transition-all duration-300 shadow-lg hover:shadow-xl disabled:opacity-60 disabled:cursor-not-allowed"
      >
